...
"""
    ...
    def setup_repo_tool(self):
        ...
        # 直接用 pip 安装 git-repo，最可靠
        import subprocess
        subprocess.run("pip install git-repo", capture_output=True)
        # 找到 repo 二进制路径
        repo_bin = "/usr/local/bin/repo"
        if not os.path.exists(repo_bin):
            repo_bin = "/usr/bin/repo"
        if not os.path.exists(repo_bin):
            # 找 pip 安装的位置
            out = subprocess.run(["python", "-m", "site", "--user-base"], capture_output=True, text=True)
            user_base = out.stdout.strip()
            repo_bin = os.path.join(user_base, "bin", "repo")
        if not os.path.exists(repo_bin):
            raise RuntimeError("找不到 repo 二进制文件")
        os.chmod(repo_bin, 0o755)
        self.env["REPO"] = repo_bin
        # 确保 PATH 包含 pip 安装目录
        pip_bin_dir = os.path.dirname(repo_bin)
        current_path = self.env.get("PATH", "")
        if pip_bin_dir not in current_path:
            self.env["PATH"] = f"{pip_bin_dir}:{current_path}"
        self.shell.env = self.env
        logger.info(f"repo 工具安装成功: {repo_bin}")
