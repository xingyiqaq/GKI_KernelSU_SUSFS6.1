...
"""
    ...
    def setup_repo_tool(self):
        ...
        # 使用 pip 安装 repo，避免 curl 下载得到损坏文件
        self._run_cmd("pip install --user git-repo", check=False)
        self.env["REPO"] = "repo"
        # 确保 ~/.local/bin 在 PATH 中
        pip_bin = str(Path.home() / ".local/bin")
        current_path = self.env.get("PATH", "")
        if pip_bin not in current_path:
            self.env["PATH"] = f"{pip_bin}:{current_path}"
        self.shell.env = self.env
