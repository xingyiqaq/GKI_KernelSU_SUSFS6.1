...
"""
    ...
    def setup_repo_tool(self):
        ...
        try:
            ...
            # 验证下载内容是否合法（首行应为 #! 开头的 shebang）
            with open(repo_path, "r") as f:
                first_line = f.readline().strip()
            logger.info(f"repo 文件首行: {first_line[:80]}")
            if not first_line.startswith("#!"):
                logger.error(f"repo 下载失败，首行不是 shebang: {first_line[:100]}")
                raise RuntimeError("repo 工具下载失败，文件内容损坏")
            self.env["REPO"] = repo_path
            self.shell.env = self.env
            logger.info(f"repo 工具安装成功: {repo_path}")
        except Exception as e:
            logger.error(f"repo 工具安装失败: {e}")
            raise
